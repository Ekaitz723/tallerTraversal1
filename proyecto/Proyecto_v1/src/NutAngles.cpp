...
void NutAngles(double Mjd_TT, double& dpsi, double& deps)
{
    ...
    // Table C: l, l', F, D, Om, dpsi, *T, deps, *T
    static const double C[106][9] = {
...
void test_NutAngles()
{
    ...
    // Test 1: J2000.0 epoch (MJD 51544.5) - T=0
    NutAngles(51544.5, dpsi, deps);
    
    // En J2000.0, la nutacion debe ser pequeña pero no cero
    assert(fabs(dpsi) < 1e-4);  // Menos de 1e-4 radianes (~20 arcseg)
    assert(fabs(deps) < 1e-4);  // Similar para oblicuidad
    
    // Test 2: Verificar que los valores estan en rangos tipicos de nutacion
    // La nutacion tipicamente varia entre ±1 arcsegundo (~±5e-6 rad)
    assert(fabs(dpsi) < 1e-3);  // Rango conservador
    assert(fabs(deps) < 1e-3);
    
    // Test 3: Epoca diferente - 1 año despues de J2000
    NutAngles(51910.0, dpsi, deps);  // ~1 año = 365.25 dias
    
    assert(fabs(dpsi) < 1e-3);
    assert(fabs(deps) < 1e-3);
    
    // Test 4: Verificar que los valores cambian con el tiempo
    double dpsi1, deps1, dpsi2, deps2;
    NutAngles(51544.5, dpsi1, deps1);      // J2000.0
    NutAngles(52000.0, dpsi2, deps2);      // ~1.25 años despues
    
    // Los valores deben ser diferentes debido al movimiento lunar/solar
    assert(fabs(dpsi1 - dpsi2) > 1e-8);
    assert(fabs(deps1 - deps2) > 1e-8);
    
    // Test 5: Epoca historica - 1980 (MJD ~44240)
    NutAngles(44239.0, dpsi, deps);  // 1 enero 1980
    
    assert(fabs(dpsi) < 1e-3);
    assert(fabs(deps) < 1e-3);
    
    // Test 6: Epoca futura - 2050 (MJD ~69807)
    NutAngles(69807.0, dpsi, deps);  // ~1 enero 2050
    
    assert(fabs(dpsi) < 1e-3);
    assert(fabs(deps) < 1e-3);
    
    // Test 7: Verificar periodicidad aproximada (18.6 años = ciclo principal)
    // La nutacion tiene componentes con periodo 18.6 años
    double periodo_dias = 18.6 * 365.25;
    NutAngles(51544.5, dpsi1, deps1);
    NutAngles(51544.5 + periodo_dias, dpsi2, deps2);
    
    // Los valores deben ser similares pero no identicos (otras periodicidades)
    assert(fabs(dpsi1 - dpsi2) < 1e-4);  // Deberian estar relativamente cerca
    assert(fabs(deps1 - deps2) < 1e-4);
    
    // Test 8: Casos con T negativo (antes de J2000)
    NutAngles(45000.0, dpsi, deps);  // T negativo grande
    
    assert(fabs(dpsi) < 1e-3);
    assert(fabs(deps) < 1e-3);
    
    // Test 9: Verificar comportamiento con casos limite
    NutAngles(100000.0, dpsi, deps);  // Epoca muy futura
    
    // Incluso en epocas extremas, debe dar valores razonables
    assert(fabs(dpsi) < 1e-2);  // Rango mas amplio para casos extremos
    assert(fabs(deps) < 1e-2);
    
    // Test 10: Verificar que dpsi y deps son independientes
    // (diferentes coeficientes en la tabla)
    NutAngles(52000.0, dpsi, deps);
    
    // No deben ser identicos (usan diferentes columnas de coeficientes)
    assert(fabs(dpsi - deps) > 1e-10);
}

REGISTER_TEST(test_NutAngles);